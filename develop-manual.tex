...
keychain会起一个ssh-agent，后来登录的人通过设置环境变量使用同一个ssh-agent.登录Shell时启动keychain:
...
http {
	...
	#rate=1000r/s 的意思是允许1秒钟不超过1000个请求
	limit_req_zone $binary_remote_addr zone=allips:100m rate=1000r/s; 
	#限制每分钟不超过600个请求
	limit_req_zone $binary_remote_addr zone=allips:100m rate=600r/m;
	server {
		limit_conn one 100;
...
server {
	location / {
		root   /opt/app/frontend-v1.3/dist;
...
\subsection{Maven Attention}

项目中在使用forEeach的Lambda表达式时，总是提示运行时错误：Error creating entityManagerFactory due to error tying to scan jar-file。Eclipse的项目设置明明是设置的jdk8.后来发现项目的Maven设置的是jdk1.7版本。
...
还有一种说法是hibernate-entitymanager的版本升级到4.2.0可以解决。Fixed it by updating the hibernate-entitymanager artifact version from 4.2.0.Final to 4.3.8.Final.当前项目的hibernate-entitymanager版本也不符合要求。
...
如果项目文件夹下没有gradlew文件，可以通过如下命令生成：

\begin{lstlisting}[language=Bash]
gradle wrapper
\end{lstlisting}

其本质是调用build.gradle中的wrapper　task:
